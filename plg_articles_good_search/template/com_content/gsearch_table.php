...
<style>
	...
	@media (max-width: 460px) {
		.table-gsearch .itemlist > div > div {
			padding: 10px 5px;
			font-size: 11px;
		}
		.table-gsearch .itemlist > div {
...
<script>
	jQuery(document).ready(function($) {
		var sort_active = "<?php echo JRequest::getVar("orderby"); ?>"; 
		var direction_active = "<?php echo JRequest::getVar("orderto", $model->module_params->ordering_default_dir); ?>";
	...
	<div class="itemlist<?php if($columns > 1) { echo ' row-fluid table'; } ?>">
		<div class="thead">
			...
			<?php foreach($sortingFields as $field) { ?>
				<?php if($field->id) { ?>
				<div class="custom-field field<?php echo $field->id; ?>" data-val="field<?php echo $field->id; ?>"><?php echo urldecode($field->name); ?></div>
				<?php } else {
					switch($field) {
						case "title" : ?>
		...
		<?php foreach($items as $items_counter=>$item) {		
				...
				$aCategories = $model->getItemCategories($item);
				...
				if($item->gsearch_item_type == 'contacts') {
					JLoader::register('ContactHelperRoute', JPATH_SITE . '/components/com_contact/helpers/route.php');
					$aCategories = $model->getItemCategories($item, 'contacts');
					$item->title = $item->name;
					$item->item_link = JRoute::_(ContactHelperRoute::getContactRoute($item->slug, $item->catid));
				}
		?>
		<div class="table-item">
			...
			if(class_exists('FieldsHelper')) {
				if($item->gsearch_item_type == 'contacts') {
					$aFields = FieldsHelper::getFields('com_contact.contact', $item, true);
				}
				else {
			...
			foreach($sortingFields as $field) {
				if($field->id) {
					...
					if($isEmpty) {
						?>
							<div class="custom-field field-empty"></div>
						<?php
					}
				} else {
					switch($field) {
