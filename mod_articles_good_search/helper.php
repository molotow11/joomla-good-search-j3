...
class modArticlesGoodSearchHelper {
	...
	function __construct($params = null) {
		if(!$params && JRequest::getInt("moduleId")) { // front
	...
	function getCategories($parent = 0, $params = null, $ordering = 'title ASC') {
		$db = JFactory::getDBO();
		$categories = Array();
		...
		if($this->params->get('search_contacts_enabled')) {
			$extensions[] = "'com_contact'";
		}
		
		if($parent) {
			$query = "SELECT * FROM #__categories WHERE extension IN(".implode(",", $extensions).") AND parent_id = {$parent} AND published = 1 ORDER BY {$ordering}";
		}
		else {
			...
			if($params && count($categories_restriction)) {
				$query = "SELECT * FROM #__categories WHERE extension IN(".implode(",", $extensions).") AND id IN (".implode(",", $categories_restriction).") ORDER BY {$ordering}";				
			}
			else {
				$query = "SELECT * FROM #__categories WHERE extension IN(".implode(",", $extensions).") AND level = 1 AND published = 1 ORDER BY {$ordering}";
			}
		}
			...
			if(JFactory::getApplication()->isSite() && $params) {
				if($params->get("restrict")) {
					if($params->get("restsub")) {
						$subs = (array)$this->getCategories($category->id, $params, $ordering);
						if(count($subs)) {
		...
		switch($params->get("restmode")) {
			...
			case 1 : //auto
				if(in_array(JRequest::getVar("view"), Array("featured", "category"))) {
					$categories[] = JRequest::getInt("catid") ? JRequest::getInt("catid") : JRequest::getInt("id");
				}
	...
	function getTags($params) {
		$db = JFactory::getDBO();				
		$query = "SELECT t.id, t.title, t.parent_id, tm.type_alias, 
					GROUP_CONCAT(DISTINCT(IF(tm.type_alias = 'com_contact.contact', c.catid, i.catid))) as catid 
				  FROM #__tags as t";
		$query .= " LEFT JOIN #__contentitem_tag_map AS tm ON t.id = tm.tag_id";
		$query .= " LEFT JOIN #__content 		 as i ON i.id = tm.content_item_id";
		$query .= " LEFT JOIN #__contact_details as c ON c.id = tm.content_item_id";
		
		$query .= " WHERE t.published = 1 AND (";
			$query .= "tm.type_alias = 'com_content.article'";
			if($this->params->get('search_contacts_enabled')) {
				$query .= " OR tm.type_alias = 'com_contact.contact'";
			}
		$query .= ")";
	...
	function getAuthors($params) {
		...
		if(count($items)) {
			...
			$query = "SELECT id, name FROM #__users WHERE id IN (".implode(',', $authors).") ORDER BY name ASC";
		}
		else {
	...
	function getFieldValuesFromText($field_id, $type = "int", $module_id) {
		$db = JFactory::getDBO();
		$query = "SELECT i.id, i.title, i.catid, GROUP_CONCAT(DISTINCT field{$field_id}.value SEPARATOR '|') as value";
		...
		$field = $db->setQuery("SELECT * FROM #__fields WHERE id = {$field_id}")->loadObject();
			...
			foreach($result as $item) {
				if($item->value) {
					...
					foreach($value as $val) {
						$res = $type == "text" ? $val : preg_replace("/[^\d+,.]/smix", "", $val);
						$res = str_replace(",", ".", $res);
						if($field->type == "articles") { // Linked Articles custom field type
							$title = $db->setQuery("SELECT title FROM #__content WHERE id = {$val}")->loadResult();
							$res .= "::" . $title;
						}
						$return[] = $res;
					}
