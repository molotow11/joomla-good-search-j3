...
class JFormFieldFilters extends JFormField {	
	...
	function fetchElement($name, $value, &$node, $control_name) {
			...
			$query = "SELECT f.*, g.title as group_name FROM #__fields as f 
						...
						WHERE f.context = 'com_content.article' and f.state = 1
			...
			// Joomla Contacts fields
			$mitems[] = JHTML::_('select.option', '', JText::_('-- Joomla Contacts Custom Fields --'));
			$query = "SELECT f.*, g.title as group_name FROM #__fields as f 
						LEFT JOIN #__fields_groups AS g ON f.group_id = g.id
						WHERE f.context = 'com_contact.contact' AND f.state = 1
						ORDER BY g.id, f.label
					";
			try {
				$db->setQuery($query);
