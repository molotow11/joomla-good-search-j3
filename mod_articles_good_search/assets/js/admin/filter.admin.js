...
$JQ(document).ready(function() {
	...
	$JQ('.ValueSelect').on('change', function() {
		...
		if(selected.val() != '' && selected.val() != 0) {
			...
			var parts = selected_val.split(":");
			if(parts.length == 5) {
				parts[4] = parts[4].replace("'", '%' + "'".charCodeAt(0).toString(16)); // escape quote character in field title
				selected_val = parts.join(":");
			}
			init_box.parent().find(".sortableFields").append("<li><span class='val' rel='"+selected_val+"'>"+ 
